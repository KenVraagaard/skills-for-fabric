...
- **CRM** (Salesforce, Dynamics): wide, sparsely-populated objects; mature managed connectors exist — Fivetran-style ELT is usually the lowest-effort path. **But Fivetran-landed tables must not have CDF enabled** (turning it on forces a billable full re-sync), so CRM MLVs sourced via Fivet